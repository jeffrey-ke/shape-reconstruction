...
def save_plot(thresholds, avg_f1_score, args):
    ...
    plt.savefig(f'eval_{args.type}_{args.tag}', bbox_inches='tight')
...
def evaluate_model(args):
    ...
    if args.load_checkpoint: # I should provide a path
    ...
    #collector = TensorCollector(fname="resnet_features")
    for step in range(start_iter, max_iter):
        ...
        # depending on what arg is given, I need to render things slightly differently. 

        mesh_gt = mesh_gt.cuda()
        mesh_add_texture(mesh_gt)
        if args.type == "vox":
            ...
            gif_up(predictions, dir="vis/mesh", name="mesh{}_{}".format(step, args.tag))
            gif_up(mesh_gt, dir="vis/mesh", name="mesh_gt{}_{}".format(step, args.tag))
